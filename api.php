...
if($from == "titleupload")
{
    ...
    $filename .= ".".$ext;
    copy($_FILES["file"]["tmp_name"],"./images/".$filename);
    mysqli_query($link, "insert into title value(null,'".$filename."', '".$_POST["text"]."', 0)");
...
elseif($from == "title")
{
    ...
    {
        mysqli_query($link,"update title set text = '".$_POST["text"]["$i"]."' where id = '".$text["$i"]."'");
    }                                                                                                         
...
{
    $ad = $_POST["text"];
    mysqli_query($link,"insert into advert value(null,'".$ad."',1)");
...
elseif($from == "mvimupload")
{
    ...
    $filename .= ".".$ext;
    copy($_FILES["file"]["tmp_name"],"./images/".$filename);
    mysqli_query($link, "insert into animate value(null,'".$filename."', 1)");
...
{   
    ...
    //echo $filename;
    copy($_FILES["file"]["tmp_name"],"./images/".$filename);
    mysqli_query($link, "update animate set file = '".$filename."' where id = '".$_GET["id"]."'");
...
elseif($from == "imageupload")
{
    ...
    $filename .= ".".$ext;
    copy($_FILES["file"]["tmp_name"],"./images/".$filename);
    mysqli_query($link, "insert into gallery value(null,'".$filename."', 1)");
...
elseif($from == "newsadd")
{
    mysqli_query($link,"insert into news value(null,'".$_POST["text"]."',1)");
...
{
    mysqli_query($link,"insert into admin value(null,'".$_POST["acc"]."','".$_POST["ps"]."',1)");
...
elseif($from == "admin")
{
    ...
    {
        mysqli_query($link,"update admin set acc = '".$_POST["acc"][$i]."' , pass = '".$_POST["ps"][$i]."' where id = '".$_POST["id"][$i]."'");
    }
    header("location:admin.php?redo=admin");
}
elseif($from == "menu")
...
elseif($from == "menuupdate")
{
    ...
    {
        if($_POST["del2"][$i] != "on")
            mysqli_query($link,"insert into menu value(null,'".$_POST["text2"][$i]."','".$_POST["href2"][$i]."',1,'".$_GET["id"]."')");
    }
    header("location:admin.php?redo=menu");
...
elseif($from == "menuadd")
{
    $text = $_POST["text"];
    $href = $_POST["href"];
    mysqli_query($link,"insert into menu value(null,'".$text."','".$href."',1,0)");
    header("location:admin.php?redo=menu");
}
